...
# -------------------------------------------------------------------------
"""_tools_provenance_v4.py —— 重建数据集溯源表（v4；N=863）"""
import os
import json
import pandas as pd
...
rows = [
    ('Figure 1 (PRISMA flow diagram)', 'v4 (n = %d)' % N,
    ...
    ('Figure 2a (publication trend)', 'v4 (n = %d)' % N,
    ...
    ('Figure 2d (dental disciplines)', 'v4 (n = %d)' % N,
    ...
    ('Figure 3 (software landscape)', 'v4 (n = %d)' % N,
    ...
    ('Figure 4 (workflow archetypes)', 'v4 (n = %d)' % N,
    ...
    ('Figure 5 (contingency analysis)', 'v4 (n = %d)' % N,
    ...
    ('Figure 6 (advantages, challenges, gaps)', 'v4 (n = %d)' % N,
    ...
    ('Supplementary File 3 (included studies and glossary)', 'v4 (n = %d)' % N,
    ...
    ('Full-text re-assessment pass', 'excluded records, n = 1,168',
     ...
     % (P['fulltext_recheck']['records_reclassified_as_eligible'],
        '{:,}'.format(P['fulltext_recheck']['excluded_records_without_full_text']))),
    ('Reverse check of the previously included set', 'v3 to v4',
     ...
     'removed because their publication date fell outside the prespecified window'),
    ('Eligibility audit', 'v3 to v4',
     '613 records assessed at full text during the initial screening, 47 excluded; the included set '
     'then changed from 566 to %d through the full-text re-assessment plus the reverse check' % N),
]
