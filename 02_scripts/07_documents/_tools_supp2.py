...
with pd.ExcelWriter(xlsx, engine='openpyxl') as w:
    ...
    # 6/7：抽样复核的逐条结论（2026-09 修订阶段补入）
    svp = os.path.join(ANA, '核验复核_逐条.csv')
    if os.path.exists(svp):
        sv = pd.read_csv(svp, encoding='utf-8-sig', low_memory=False)
        svA = sv[sv['样本'] == 'A 排除抽样'].copy()
        svB = sv[sv['样本'] == 'B 纳入抽样'].copy()
        svA.to_excel(w, sheet_name='6_Sample_verification_excluded', index=False)
        svB.to_excel(w, sheet_name='7_Sample_verification_included', index=False)
    wb = w.book
